...
class AdapterWithAuxHead(nn.Module):
    ...
    def forward(
        ...
        if lengths is None:
            ...
            plen = (lengths.to(prefix.device).float() / ratio).ceil().long().clamp(1, n_out)
            m = (torch.arange(n_out, device=prefix.device).unsqueeze(0)
                 < plen.unsqueeze(1)).to(prefix.dtype)       # (B, N) 1 = real
            pooled = (prefix * m.unsqueeze(-1)).sum(1) / m.sum(1).clamp(min=1.0).unsqueeze(-1)
        if self.reliability_head:
