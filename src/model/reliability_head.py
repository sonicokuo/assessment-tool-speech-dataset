...
def heteroscedastic_nll(
    ...
    Per feature i:
        L_i = (σ_i²)^β_[stop-grad] * ( 0.5 * exp(-s_i) * ((y_i - μ_i)/scale_i)^2 + 0.5 * s_i )
    with s_i = log σ_i². The squared-error term is normalized by `scale_i` (robust
    per-feature dispersion) so it is unit-free and comparable across features, mirroring
    ...
    β-NLL (Seitzer et al., "On the Pitfalls of Heteroscedastic Uncertainty Estimation
    with Probabilistic Neural Networks", ICLR 2022): the plain NLL's exp(-s) factor
    down-weights the squared-error gradient on high-σ points, so the network stops
    fitting exactly the hard (high-uncertainty) features it most needs to learn — the
    "gets lazy on hard points" pathology. Multiplying each term by a *detached*
    σ^{2β} = exp(β·s) counteracts that: β=1 recovers a plain-MSE-like error gradient
    magnitude while still learning σ; β=0 is the original Kendall & Gal NLL. Seitzer
    recommends β≈0.5. The weight is stop-gradient, so it re-weights the loss without
    changing the objective's stationary point.
    ...
    # β-NLL (Seitzer 2022): re-weight each term by a *detached* σ^{2β} = exp(β·s) so the
    # squared-error gradient is not suppressed on high-σ features. β=0 -> no-op.
    if beta != 0.0:
        per_feat = per_feat * torch.exp(float(beta) * log_var).detach()
