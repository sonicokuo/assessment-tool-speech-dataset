...
SECTION_TAGS: tuple[SectionTag, ...] = (
    ...
    SectionTag(
        name="voice", tag="<sec_voice>",
        feature_names=("hnr", "jitter", "shimmer"),
        display_name="voice quality",
    ),
    SectionTag(
...
FEATURE_TAGS: tuple[FeatureTag, ...] = (
    FeatureTag(
        ...
        template="the signal-to-noise ratio SNR is {value} dB, a {qual} recording",
        section="noise",
    ),
    FeatureTag(
        ...
        template="the signal is {qual}, with an SRMR of {value}",
        section="reverb",
    ),
    FeatureTag(
        ...
        template="the F0 mean is {value} Hz, a {qual} voice",
        section="pitch",
    ),
    FeatureTag(
        ...
        template="the F0 standard deviation SD is {value} Hz, {qual} intonation",
        section="pitch",
    ),
    FeatureTag(
        ...
        template="the speaking rate is {value} syl/sec, a {qual} pace",
        section="tempo",
    ),
    FeatureTag(
    ...
    FeatureTag(
        ...
        template="the overlap ratio is {value}, {qual} overlap",
        section="overlap",
    ),
    FeatureTag(
    ...
    FeatureTag(
        name="hnr", tag="<f_hnr>",
        csv_col="hnr", sfs_key="hnr",
        unit="dB", display_name="harmonics-to-noise ratio",
        template="the harmonics-to-noise ratio HNR is {value} dB, a {qual} voice",
        section="voice",
    ),
    FeatureTag(
        name="jitter", tag="<f_jitter>",
        csv_col="jitter_local_pct", sfs_key="jitter",
        unit="%", display_name="jitter",
        template="the jitter local is {value} percent ({qual})",
        section="voice",
    ),
    FeatureTag(
        name="shimmer", tag="<f_shimmer>",
        csv_col="shimmer", sfs_key="shimmer",
        unit="%", display_name="shimmer",
        template="the shimmer local is {value} percent ({qual})",
        section="voice",
    ),
)
...
# ── Verbalizer helpers ──────────────────────────────────────────────────────
# ── Qualitative bands (grounded thresholds; see REBUILD_SPEC.md "VERBALIZER RULE TABLE") ──
# Physical anchors where they exist (SNR: BS5839-8 10 dB floor; f0: NCVS ~120/210 Hz); data-relative
# quartile bins where the literature proves NO physical threshold exists (SRMR, rate, voice quality).
# Each entry: list of (upper_bound, adjective); value < upper_bound picks the adjective.
FEATURE_BANDS: dict[str, list] = {
    "snr":           [(10.0, "noisy"), (20.0, "fair"), (30.0, "good"), (float("inf"), "clean")],
    "srmr":          [(2.5, "heavily reverberant"), (4.7, "moderately reverberant"), (float("inf"), "lightly reverberant")],
    "f0_mean":       [(150.0, "low-pitched"), (200.0, "mid-pitched"), (float("inf"), "high-pitched")],
    "f0_sd":         [(30.0, "monotone"), (60.0, "moderately varied"), (float("inf"), "expressive")],
    "speaking_rate": [(4.5, "slow"), (6.0, "measured"), (float("inf"), "brisk")],
    "overlap_ratio": [(0.001, "no"), (0.3, "light"), (0.6, "moderate"), (float("inf"), "heavy")],
    "jitter":        [(1.9, "low"), (2.7, "moderate"), (float("inf"), "high")],
    "shimmer":       [(8.2, "low"), (11.2, "moderate"), (float("inf"), "high")],
    "hnr":           [(9.6, "breathy"), (13.6, "moderately clear"), (float("inf"), "clear")],
}
def _feature_band(name: str, v: float) -> str:
    for upper, adj in FEATURE_BANDS.get(name, []):
        if v < upper:
            return adj
    return ""
...
def build_section_body(section_name: str, feature_values: dict[str, str]) -> str:
    ...
    for fname in sec.feature_names:
        ...
        try:
            _qual = _feature_band(fname, float(feature_values[fname]))
        except (ValueError, TypeError):
            _qual = ""
        body = ft.template.format(value=feature_values[fname], qual=_qual)
