#!/usr/bin/env python3
"""SFS-vs-human correlation study — validates the SFS metric.

When you propose a new automatic metric for a paper, reviewers want to
know it correlates with human judgement. This script supports a two-phase
workflow:

  PHASE 1 (--mode prepare):
    Sample N clips from inference_results.json, write a rating-ready CSV
    with one row per clip: [filename, target, generated, sfs_f1, human_rating].
    The human_rating column is blank. You (or co-authors / Mturk) fill it
    in with a 1-5 faithfulness score using the rubric printed by the script.

  PHASE 2 (--mode analyze):
    Read the filled CSV. Compute Spearman + Pearson correlation between
    SFS-F1 and the human rating. Plot a scatter and save a markdown
...
-----
    ...
    python scripts/sfs_human_correlation.py --mode prepare \
      ...
      --output_csv        $SHARED/checkpoints/v7_lora_8b/human_ratings.csv \
      --n 50 --seed 42

    # Open the CSV, fill in the human_rating column (integer 1-5)

    # Phase 2 — analyze
    python scripts/sfs_human_correlation.py --mode analyze \
      --rated_csv  $SHARED/checkpoints/v7_lora_8b/human_ratings.csv \
      --output_md  $SHARED/checkpoints/v7_lora_8b/sfs_human_correlation.md
"""
...
'human_rating' column with an integer 1-5.
"""
...
def prepare(args) -> int:
    ...
    # Stratify by sfs_f1 quartiles so the rater sees a range of quality
    ...
    with args.output_csv.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["filename", "sfs_f1", "target", "generated", "human_rating"])
        for r in sample:
            w.writerow([
                r["filename"],
                f"{r['sfs_f1']:.3f}",
    ...
    print(f"Done. Open {args.output_csv} in a spreadsheet and fill the "
          f"'human_rating' column (1-5), then run --mode analyze.")
    return 0
...
def analyze(args) -> int:
    ...
    for r in rows:
        try:
            sfs = float(r["sfs_f1"])
            human = float(r["human_rating"])
            rated.append({"filename": r["filename"], "sfs": sfs, "human": human,
                          "target": r.get("target", ""), "generated": r.get("generated", "")})
        except (ValueError, KeyError):
            continue
    if len(rated) < 5:
        print(f"ERROR: only {len(rated)} clips with valid human_rating. Need ≥5.")
        return 2
    sfs = [r["sfs"] for r in rated]
    human = [r["human"] for r in rated]
    n = len(rated)

    spearman = st.spearmanr(sfs, human)
    pearson = st.pearsonr(sfs, human)

    # Per-bin means: bucket SFS into thirds, report mean human rating per third
    ...
    for i, idxs in enumerate(bins):
        ...
        bin_human = [human[j] for j in idxs]
        bin_stats.append({
            "name": ["low SFS", "mid SFS", "high SFS"][i],
            "n": len(idxs),
            "sfs_range": (min(bin_sfs), max(bin_sfs)),
            "mean_human": sum(bin_human) / len(bin_human),
        })

    md = [
        "# SFS-vs-human correlation",
        "",
        f"**N rated clips:** {n}",
        "",
        "## Correlation",
        ...
        "",
        "## Mean human rating per SFS tercile",
        "",
        "| Bucket | n | SFS range | Mean human rating |",
        "|---|---:|---|---:|",
    ]
    for b in bin_stats:
        md.append(f"| {b['name']} | {b['n']} | {b['sfs_range'][0]:.3f}–{b['sfs_range'][1]:.3f} | {b['mean_human']:.2f} |")
    md.extend([
        "",
        "## Interpretation",
        "",
        ...
        "- The terciles table shows whether the metric tracks human judgement",
        "  even at the coarse high/mid/low level — a strict monotone increase in",
        "  mean human rating across terciles is the cleanest qualitative evidence.",
    ])
